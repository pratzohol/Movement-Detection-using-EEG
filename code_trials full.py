...
for subjects in range(1,6):
  for fff in range(1,11):
    ...
    if fff<10:
      data = mne.io.read_raw_gdf('S0'+str(subjects)+'_MI/motorimagination_subject'+str(subjects)+'_run'+str(fff)+'.gdf')
    else:
      data = mne.io.read_raw_gdf('S0'+str(subjects)+'_MI/motorimagination_subject'+str(subjects)+'_run'+str(10)+'.gdf')
