...
def feature_normalize(data):
    ...
    data_normalized[data_normalized.nonzero()] = (
        data_normalized[data_normalized.nonzero()] - mean)/sigma
    # return shape: 9*9
...
def apply_mixup(dataset_file, window_size, label, yes_or_not):  # initial empty label arrays
    ...
    for trial in range(0, trials):
        ...
        data, label = segment_signal_without_transition(
            data, label_in, label_index, window_size)
        ...
        # rnn data process
        data_rnn = data. reshape(
            int(data.shape[0]/window_size), window_size, 32)
